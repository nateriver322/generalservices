...
const TicketRow = React.memo(({ 
  ...
  <TableRow>
    ...
    <TableCell style={{ color: STATUS_COLORS[ticket.status] || 'black' }}>
      {ticket.status}
    </TableCell>
...
const CustomModal = React.memo(({ open, onClose, title, children }) => (
  <Modal open={open} onClose={onClose}>
    <Box sx={{ ...MODAL_STYLE, width: '80%', maxWidth: 500 }}>
...
function TicketsCreated() {
  ...
  const openModal = useCallback((modalName, ticket = null) => {
  ...
  const handleAssignTicket = useCallback((ticket) => {
    setSelectedTicket(ticket);
    const workTypes = ticket.workType.split(',');
    setFilteredPersonnel(personnelList.filter(
      personnel => workTypes.includes(personnel.subrole)
    ));
    setSelectedPersonnel([]);
    openModal('assign');
  }, [personnelList, openModal]);
  ...
  const handleAssignPersonnel = useCallback(async () => {
    ...
    try {
      const response = await axios.post(
        'https://generalservicescontroller.onrender.com/api/tickets/assign',
        null,
        {
          params: {
  ...
  }, [selectedTicket, selectedPersonnel, scheduledRepairDate, closeModal, openModal, fetchData]);

  const handleStaffFeedbackSubmit = useCallback(async () => {
    if (!selectedTicket?.id || !staffFeedback) return;

    try {
      const response = await axios.post(
        `https://generalservicescontroller.onrender.com/api/tickets/${selectedTicket.id}/staff-feedback`,
        {
  ...
  }, [selectedTicket, staffFeedback, closeModal, openModal]);
  ...
  const handleDeleteTicket = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `https://generalservicescontroller.onrender.com/api/tickets/${selectedTicket.id}`,
        { method: 'DELETE' }
      );
